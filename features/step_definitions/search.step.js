...
When("user chose a time {string}", {timeout: 30 * 10000}, async function (string) {
...
When("The reserve", {timeout: 30 * 10000}, async function () {
    const isDisabled = await this.page.$eval("button", (button) => {
        return button.disabled;
      });
});
...
Then("user check the session {string}", {timeout: 30 * 10000}, async function (string) {
